...
let cachedGlobalScript: string | null = null;

/**
 * Get the global ad script
 */
export const getGlobalAdScript = (): string | null => {
...
export const fetchActiveAds = async (): Promise<Ad[]> => {
  try {
    ...
    // Store global script if present
    if (data.globalScript !== undefined) {
      cachedGlobalScript = data.globalScript;
    }

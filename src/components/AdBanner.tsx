...
import { fetchActiveAds, getAdsByPosition, getRandomAd, getGlobalAdScript, Ad } from "../services/adService";
...
const AdBanner = ({ adSlot, adId, className = "", position = "between", fullRow = false }: AdBannerProps) => {
  ...
  useEffect(() => {
    ...
    const loadAd = async () => {
      try {
        ...
        // Get global script
        const script = getGlobalAdScript();
  ...
  useEffect(() => {
    if (!globalScript || !scriptRef.current) return;
    ...
    adContainer.innerHTML = globalScript;
  ...
  if (!ad && !globalScript && AD_CONFIG.PROVIDER !== 'google-adsense') {
    ...
    >
      {globalScript ? (
        <div className="ad-content" ref={scriptRef}>
          {/* Global ad script will be injected here */}
        </div>
      ) : (
        <div className="ad-placeholder">
